let audio = null;
let playlist = [];
let currentIndex = Number(localStorage.getItem("anhsTrack") || 0);
let playlistReady = false;
let audioEventsAttached = false;

const RADIO_DEFAULT_COVER = "anhs-club-logo.png";
const RADIO_PLAYLIST_URL = `data/playlist.json?v=${Date.now()}`;
...
function initializeRadio() {
  ...
  const savedVolume = Number(
    localStorage.getItem("anhsVolume")
  );
...
function loadTrack(nextIndex, restoreTime = true) {
  ...
  if (restoreTime) {
    const savedTime = Number(
      localStorage.getItem("anhsTime") || 0
    );
...
function syncRadioUI() {
  ...
  const artist =
    document.getElementById("radioArtist");
...
function bindRadioControls() {
  ...
  const previous =
    document.getElementById("radioPrev");

  const next =
    document.getElementById("radioNext");
...
function attachAudioEvents() {
  ...
  player.addEventListener(
    ...
    () => {
      ...
      console.error(
        "Unable to load ANHS Radio track:",
        track?.file,
        player.error
      );
      ...
      if (artist) {
        artist.textContent =
          "Audio file is missing, empty, or invalid.";
      }
    }
  );
